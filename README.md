...
* supported arguments, passed as an array
  * `display_private`: (*boolean*) Include user events marked as Private. Default is `false`. Note that this changes the endpoint called from `event_search` to `user_events`. See the [Eventbrite API docs](https://www.eventbrite.com/developer/v3/) for details.
